...
CREATE TABLE TB_VAGA_GARAGEM_LOG(
 ...
 DT_EXECUTOR DATE CONSTRAINT NN_TB_VAGA_GARAGEM_LOG_DT_EXECUTOR NOT NULL,
...
CREATE TABLE TB_PF_TELEFONE_LOG(
 ...
 DT_EXECUTOR DATE CONSTRAINT NN_TB_PF_TELEFONE_LOG_DT_EXECUTOR NOT NULL,
...
CREATE TABLE TB_PJ_TELEFONE_LOG(
 ...
 DT_EXECUTOR DATE CONSTRAINT NN_TB_PJ_TELEFONE_LOG_DT_EXECUTOR NOT NULL,
	...
	IF (existe3 = 1) THEN
		...
		IF (existe2 = 0) THEN
			...
			IF (existe = 0) THEN
				...
				SELECT CAST(nextval('seq_cd_usuario') AS CHAR)
				INTO cd_usu_v;
				
	
				EXECUTE 'CREATE USER "' || cd_usu_v || '" WITH PASSWORD ''' || $2 || '''';
	
				IF ($3 = 1) THEN 
		
					EXECUTE 'ALTER GROUP administrador ADD USER "' || cd_usu_v || '"';	
					
					EXECUTE 'ALTER USER "' || cd_usu_v || '" CREATEUSER';
				
				ELSIF ($3 = 2) THEN
		
					EXECUTE 'ALTER GROUP atendente ADD USER "' || cd_usu_v || '"';	
					
					EXECUTE 'ALTER USER "' || cd_usu_v || '" NOCREATEUSER';
	
				ELSIF ($3 = 3) THEN
		
					EXECUTE 'ALTER GROUP usuario ADD USER "' || cd_usu_v || '"';
		...
		ELSE
			...
			IF (existe4 = 0) THEN
				...
				IF ($2 IS NOT NULL) THEN				
					...
					EXECUTE 'ALTER USER "' || cd_usu_v2 || '" WITH ENCRYPTED PASSWORD ''' || $2 || '''';
				...
				IF($3 IS NOT NULL) THEN
					...
					UPDATE tb_usuario SET	
						nivel = $3,
						CD_USUARIO_CRIACAO = (select cd_USUARIO 
										      from tb_usuario 
										      where cd_usu_banco = user),
						DT_USUARIO_CRIACAO = now(),
						CD_USUARIO_ATUALIZA = (select cd_USUARIO 
										       from tb_usuario 
							                   where cd_usu_banco = user),
					...
					IF (aux_nivel = 1) THEN 
						...
						EXECUTE 'ALTER GROUP administrador DROP USER "' || cd_usu_v2 || '"';
			
					ELSIF (aux_nivel = 2) THEN
		
						EXECUTE 'ALTER GROUP atendente DROP USER "' || cd_usu_v2 || '"';	
							
					ELSIF (aux_nivel = 3) THEN
		
						EXECUTE 'ALTER GROUP usuario DROP USER "' || cd_usu_v2 || '"';
		
					END IF;		
		
					IF ($3 = 1) THEN 
		
						EXECUTE 'ALTER GROUP administrador ADD USER "' || cd_usu_v2 || '"';
						
						EXECUTE 'ALTER USER "' ||cd_usu_v2 || '" CREATEUSER';
			
					ELSIF ($3 = 2) THEN
		
						EXECUTE 'ALTER GROUP atendente ADD USER "' || cd_usu_v2 || '"';	
						
						EXECUTE 'ALTER USER "' || cd_usu_v2 || '" NOCREATEUSER';
		
					ELSIF ($3 = 3) THEN
		
						EXECUTE 'ALTER GROUP usuario ADD USER "' || cd_usu_v2 || '"';	
